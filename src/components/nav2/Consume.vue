<template>
  <div id="Payment">
    <el-card class="box-card">
      <el-table
        ...
        <el-table-column
          ...
          <template slot-scope="scope">
            <div class="time_size">{{scope.row.paymentDate | data}} {{scope.row.paymentDate | time}}</div>
          </template>
        </el-table-column>
        <el-table-column
          prop="payment"
      ...
      <div class="sum">消费总额:<span>{{costSum}}</span>元</div>
    ...
    <el-dialog title="详情" :visible.sync="dialogVisible1" width="90%" top="20vh">
      ...
      <dl class="text xqfl">
        <dd class="item1 xqfl1">消费金额:</dd>
        <dd class="item xqfl2">{{payment}}</dd>
      </dl>
      <dl class="text xqfl">
        <dd class="item1 xqfl1">消费时间:</dd>
        <dd class="item xqfl2">{{paymentDate}}</dd>
      </dl>
      <dl class="text xqfl">
      ...
      cardConsume.then((res) => {
        if(res.data.msg == 'OK'){
          ...
          for(let i=0;i<data.length;i++){
            costSum += parseFloat(data[i].payment);
